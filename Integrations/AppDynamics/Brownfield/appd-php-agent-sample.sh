...
mkdir appd-php
cd /tmp/appd-php
wget ${AGENT_LOCATION}/php-agent/appdynamics-php-agent-x64-linux-4.3.5.0.tar.bz2; touch /tmp/appd-agent/downloadagent.done &
...
chmod 777 logs/
sudo ./install.sh -s -a=${APPD_ACCOUNT_NAME}@${APPD_ACCESS_KEY} $APPD_CONTROLLER $APPD_CONTROLLER_PORT $APP_NAME $TIER_NAME $HOSTNAME
...
echo "AppDynamics App Agent Installation...done!"
